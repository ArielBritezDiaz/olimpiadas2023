...
<body>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $("#enfermero").autocomplete({
                <?php
                    require("conexion.php");

                    $query = "SELECT * FROM Enfermero WHERE apellido LIKE '{$_GET['#enfermero']}%' LIMIT 25";
                    $result = mysqli_query($conexion, $query);
                    if(mysqli_num_rows($result) > 0) {
                        while($enfermero = mysqli_fetch_array($result)) {
                            $res[] = $enfermero['apellido'];
                        }
                    } else {
                        $res = array();
                    }
